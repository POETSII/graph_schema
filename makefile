...
CURRENT_MAKEFILE_DIR := $(notdir $(patsubst %/,%,$(dir $(CURRENT_MAKEFILE_PATH))))

# Look for external interface spec
ifeq ($(POETS_EXTERNAL_INTERFACE_SPEC_DIR),)
	POETS_EXTERNAL_INTERFACE_SPEC_DIR_TRY:=$(wildcard "$(CURRENT_MAKEFILE_DIR)/../external_interface_spec" )
	ifeq ($(POETS_EXTERNAL_INTERFACE_SPEC_DIR_TRY),)
		# nothing
	else
		POETS_EXTERNAL_INTERFACE_SPEC_DIR=$(POETS_EXTERNAL_INTERFACE_SPEC_DIR_TRY)
	endif
endif

# And enable externals if we find it
ifneq ($(POETS_EXTERNAL_INTERFACE_SPEC_DIR),)
	POETS_EXTERNAL_INTERFACE_SPEC_ENABLE=1
	CPPFLAGS += -I $(POETS_EXTERNAL_INTERFACE_SPEC_DIR)/include -DPOETS_EXTERNAL_INTERFACE_SPEC_ENABLE=1
else
	POETS_EXTERNAL_INTERFACE_SPEC_ENABLE=0 
	CPPFLAGS +=  -DPOETS_EXTERNAL_INTERFACE_SPEC_ENABLE=0
endif
