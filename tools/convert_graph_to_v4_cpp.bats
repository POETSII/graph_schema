...
@test "BinConvert v3 ising spin to v4" {
    run_no_stderr bin/convert_graph_to_v4 apps/ising_spin/ising_spin_8x8.xml
    [ $status -eq 0 ]
...
@test "BinConvert compressed v3 ising spin to v4" {
    run_no_stderr bin/convert_graph_to_v4 apps/ising_spin/ising_spin_8x8.xml.gz
    [ $status -eq 0 ]
    echo $output | grep '<EdgeI path="n_7_7:in-n_6_7:out" P="{4}"/>'
    echo $output | grep '</Graphs>'
}
...
@test "BinConvert compressed v3 ising spin to compressed v4" {
    WD=$(make_test_wd)
    bin/convert_graph_to_v4 apps/ising_spin/ising_spin_8x8.xml.gz $WD/graph.xml.gz
    [ -f $WD/graph.xml.gz ]
...
@test "BinRound-tripping just a graph type from v3 to v4 then back to v3 and the compiling as provider" {
    ...
    bin/convert_graph_to_v4 apps/ising_spin/ising_spin_graph_type.xml $WD/ising_spin_graph_type.v4.xml
