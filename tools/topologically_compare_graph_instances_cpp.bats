...
@test "check bin/topologically_compare_graph_instances exists" {
    [ -x bin/structurally_compare_graph_types ]
}

@test "Check different v3 graphs are topologically different/same" {
    local WD=$(get_bats_file_wd)

    for i in ${WD}/*.v3.xml ; do
        for j in ${WD}/*.v3.xml ; do
            run bin/topologically_compare_graph_instances $i $j
...
@test "Check different v4 graphs are topologically different/same" {
    local WD=$(get_bats_file_wd)
...
@test "Check v3 graphs are topologically different/same as v4" {
    local WD=$(get_bats_file_wd)
    local bi
    local bj
