...
for s in range(S):
	for z in range(Z):
		for n in range(N):	
			for m in range(M):
				print '\t<DevI id=\"firefly_s'+str(s)+'_'+str(z*Z*M)+str(n*M)+str(m)+'\" type=\"firefly\">'
	...
	for z in range(Z):
		for n in range(N):
			for m in range(M):
				...
				print '\t<EdgeI path=\"firefly_s'+str(s)+'_'+str(z*Z*M)+str(n*M)+str(m)+':tick_in-firefly_s'+str(s)+'_'+str(z*Z*M)+str(n*M)+str(m)+':tick_out\"/>'
	
				#Top Boundary z=0
				if z != 0:
					print '\t\t<EdgeI path="firefly_s'+str(s)+'_'+str(z*Z*M)+str(n*M)+str(m)+':flash_in-firefly_s'+str(s)+'_'+str((z-1)*Z*M)+str(n*M)+str(m)+':flash_out"/>'

				#Bottom Boundary z=Z-1
				if z != (Z-1):
					print '\t\t<EdgeI path="firefly_s'+str(s)+'_'+str(z*Z*M)+str(n*M)+str(m)+':flash_in-firefly_s'+str(s)+'_'+str((z+1)*Z*M)+str(n*M)+str(m)+':flash_out"/>'

				#North boundary
				if n != (N-1):
					print '\t\t<EdgeI path="firefly_s'+str(s)+'_'+str(z*Z*M)+str(n*M)+str(m)+':flash_in-firefly_s'+str(s)+'_'+str(z*Z*M)+str((n+1)*M)+str(m)+':flash_out"/>'
					                                                                                                               
				#East boundary
				if m != (M-1): 
					print '\t\t<EdgeI path="firefly_s'+str(s)+'_'+str(z*Z*M)+str(n*M)+str(m)+':flash_in-firefly_s'+str(s)+'_'+str(z*Z*M)+str(n*M)+str(m+1)+':flash_out"/>'
