...
def generate_lengthN_boundary(graphname, N, debug, properties, state, updateFunc, initFunc):
	...
	o+='\t\t\t<Scalar type=\"int8_t\" name=\"init_firstfire\"/>\n'
	o+=state
	o+='\t\t</State>\n'
	o+='\n'
	...
	o+='\t\t<ReadyToSend><![CDATA[\n'
	o+='\t\t\t*readyToSend=0;\n'
	o+='\t\t\tif((deviceState->t == 0) && (deviceState->init_firstfire == 1)) {\n'
	...
	o+='\t\t\t\tdeviceState->init_firstfire = 1;\n'
	...
	o+='\t\t\t\t\t}\n'
	o+='\t\t\t\t}\n'
	o+='\t\t\t\tdeviceState->init_firstfire=0;\n'
	o+='\t\t\t]]></OnSend>\n'
	o+='\t\t</OutputPin>\n'
	o+='\n'
...
#returns a string containing xml code describing a halo exchange N x N device
	...
	for direction in directions:
		...
		o+='\t\t\t\t\t\tcell_updateFunc(deviceState, deviceProperties);\n'
		if debug:
