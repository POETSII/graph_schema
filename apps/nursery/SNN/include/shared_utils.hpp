...
// This is intended to be inlined.
bool izhikevich_update(
    float dt,
    float a, float b, float c, float d, float i_offset, float Ir,
    uint64_t &rng,
    float &u, float &v,
    float stimulus
){
    float I=stimulus;
    if(Ir!=0.0f){
        float g = MWC64Gaussian_Tab64_CLT4(&rng);
        g = g*Ir;
        I=I+g;
    }
    ...
    {
        float vR=v+dt*(0.04*(v*v)+5*v+140-u+I+i_offset);
        ...
        float acc3a=I+i_offset;
        float acc4=acc3+acc3a;
        float acc5=acc4*dt;
        v=v+acc5;

        assert( std::abs((v-vR)/vR) < 1e-6 );
    }
    // u=u+a*(b*v-u);
    {
        float bv=b*v;
