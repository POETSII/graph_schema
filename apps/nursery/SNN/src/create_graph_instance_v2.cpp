...
void render_network_as_graph_instance_HwIdle(
    Network &net,
    ...
    bool sendHashOnSpike=config.at("sendHashOnSpike").get_value_int("1");
    ...
    if(synapseProperties->payloadSize()!=4){
        throw std::runtime_error("Expedcting 4-byte edge property with a single integer weight."); 
    }
    ...
    graphPropertiesType->setScalarSubElement("send_hash_on_spike", graphProperties, uint32_t(sendHashOnSpike));
...
void render_network_as_graph_instance_GALSExact(
    Network &net,
    GraphLoadEvents &writer
){
    std::string algorithm_name="GALSExact";
    ...
    bool dumpHashEachStep=false;
    if(config.find("dumpHashEachStep")!=config.end()){
        dumpHashEachStep=config.at("dumpHashEachStep").get_value_int("1");
    }
    ...
    auto neuronDeviceType=graphType->getDeviceType("neuron");
    auto neuronSpikeInPin=neuronDeviceType->getInput("spikem_in");
    auto neuronSpikeOutPin=neuronDeviceType->getOutput("spikem_out");
    auto neuronHashOutPin=neuronDeviceType->getOutput("hash_out");

    auto externalDeviceType=graphType->getDeviceType("external_output");
    auto externalHashInPin=externalDeviceType->getInput("hash_in");
    auto externalSpikeInPin=externalDeviceType->getInput("spikem_in");
    auto externalStopOutPin=externalDeviceType->getOutput("stop");
    ...
    uint32_t stats_export_interval=dumpHashEachStep ? 1 : max_steps;
    graphPropertiesType->setScalarSubElement("stats_export_interval", graphProperties, stats_export_interval);
    ...
    robin_hood::unordered_flat_set<uint32_t> local_bidir_links;
    robin_hood::unordered_flat_set<uint64_t> global_unidir_links;

    auto make_global_link=[&](uint32_t dst, uint32_t src)
    {
        return (uint64_t(dst)<<32) + src;
    };
    ...
    for(auto &n : net.nodes){
        ...
        // PROPS consist of nhood + neuron properties (other members now removed)
        if(dst_props->payloadSize() != src_props.second+4){
            std::stringstream tmp;
            tmp<<"Neuron properties sizes dont match. expected from graph type ="<<dst_props->payloadSize()<<", got from neuron model ="<<src_props.second+4<<"\n";
            throw std::runtime_error(tmp.str());
        }
        
        memcpy(dst_props.payloadPtr(), src_props.first, src_props.second);

        local_bidir_links.clear();
        for(const auto &e : n.outgoing){
            local_bidir_links.insert(e.dest);
            global_unidir_links.insert( make_global_link(e.dest, n.index) );
        }
        local_bidir_links.insert(n.incoming.begin(), n.incoming.end());

        uint32_t nhood=local_bidir_links.size();
        *(uint32_t*)(dst_props.payloadPtr()+src_props.second) = nhood;
    ...
    robin_hood::unordered_flat_set<uint64_t> added;
    ...
    for(auto &src : net.nodes){
        ...
        for(auto &edge : src.outgoing){
            ...
            *(int32_t*)localProperties->payloadPtr() = edge.weight;
            ...
            added.insert( (uint64_t(dst.index)<<32) + src.index );
        }
    ...
    for(auto ee : global_unidir_links){
        uint32_t dst_i=ee>>32, src_i=ee&0xFFFFFFFFul;

        if(global_unidir_links.find( make_global_link(src_i,dst_i) ) == global_unidir_links.end()){
            auto &dst = net.nodes[dst_i];
            auto &src = net.nodes[src_i];
            ...
            writer.onEdgeInstance(
                gid,
                src_sink_index, neuronDeviceType, neuronSpikeInPin,
                dst_sink_index, neuronDeviceType, neuronSpikeOutPin,
                -1, TypedDataPtr{}, TypedDataPtr{}
            );
        }
    }
...
void render_network_as_graph_instance_CountExact(
    Network &net,
    GraphLoadEvents &writer
){
    std::string algorithm_name="CountExact";
    ...
    auto neuronSpikeOutPin=neuronDeviceType->getOutput("spike_out");
    auto neuronHashOutPin=neuronDeviceType->getOutput("hash_out");
    auto neuronReduceOutPin=neuronDeviceType->getOutput("reduce_out");
    auto neuronAdvanceInPin=neuronDeviceType->getInput("advance_in");

    auto syncDeviceType=graphType->getDeviceType("sync");
    auto syncReduceInPin=syncDeviceType->getInput("reduce_in");
    auto syncAdvanceOutPin=syncDeviceType->getOutput("advance_out");
    ...
    uint64_t sync_index=writer.onDeviceInstance(gid, syncDeviceType, "sync", TypedDataPtr{}, TypedDataPtr{} );

    for(auto &n : net.nodes){
        auto dst_props=neuronDeviceType->getPropertiesSpec()->create();
        auto src_props=n.neuron->get_properties();

        // PROPS consist of fanout + neuron properties (other members now removed)
        if(dst_props->payloadSize() != src_props.second+4){
            std::stringstream tmp;
            tmp<<"Neuron properties sizes dont match. expected from graph type ="<<dst_props->payloadSize()<<", got from neuron model ="<<src_props.second+4<<"\n";
            throw std::runtime_error(tmp.str());
        ...
        memcpy(dst_props.payloadPtr(), src_props.first, src_props.second);
        *(uint32_t*)(dst_props.payloadPtr()+src_props.second) = n.outgoing.size();
    ...
    for(auto &src : net.nodes){
        ...
        writer.onEdgeInstance(gid,
                sync_index, syncDeviceType, syncReduceInPin,
                src_sink_index, neuronDeviceType, neuronReduceOutPin,
                -1, TypedDataPtr{}, TypedDataPtr{}
        );
        writer.onEdgeInstance(gid,
                src_sink_index, neuronDeviceType, neuronAdvanceInPin,
                sync_index, syncDeviceType, syncAdvanceOutPin,
                -1, TypedDataPtr{}, TypedDataPtr{}
        );
...
int main(int argc, char *argv[])
{
    ...
    if(algorithm_name=="HwIdle"){
        render_network_as_graph_instance_HwIdle(
            net_builder.net, *writer
        );
    }else if(algorithm_name=="GALSExact"){
        render_network_as_graph_instance_GALSExact(
            net_builder.net, *writer
        );
    }else if(algorithm_name=="CountExact"){
        render_network_as_graph_instance_CountExact(
            net_builder.net, *writer
        );
    }else{
        std::cerr<<"Unknown method : "<<algorithm_name<<"\n";
        exit(1);
    }
} 
