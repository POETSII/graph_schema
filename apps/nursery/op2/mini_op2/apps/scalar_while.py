...
def build_system(n:int) -> (SystemInstance,Statement):
    ...
    y=sys.create_mutable_global("y", DataType(shape=(1,)))
    ...
    instOutput={
        "output" : {
            "initial" : copy.copy(instDict),
            "final" : copy.copy(instDict)
        },
        **instDict
    }
    
    instOutput["output"]["final"]["x"] = numpy.array( [ 0.0 ] )
    instOutput["output"]["final"]["y"] = numpy.array( [ 10.0 ] )

    code=Seq(
        CheckState(instOutput, "/output/initial"),
        """
        x[0] = 10
        y[0] = 0
        """,
