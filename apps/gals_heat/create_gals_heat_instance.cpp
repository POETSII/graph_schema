...
int main(int argc, char *argv[])
{
  ...
  graphProperties->weightSelf=weightSelf;
  graphProperties->weightOther=weightOther;
  ...
  writer->onBeginGraphInstance(graphType, instName, graphProperties);

  writer->onBeginDeviceInstances();
  ...
  for(unsigned x=0;x<n;x++){
	...
	uint64_t index=writer->onDeviceInstance(dirichletType, id, props, nativeLocation);
	...
	props->initValue=drand48()*2-1;
	std::string id="c_"+std::to_string(x)+"_"+std::to_string(y);
	uint64_t index=writer->onDeviceInstance(cellType, id, props, nativeLocation);
	nodes[xyToIndex(x,y)]=std::make_pair(index,true);
	//fprintf(stderr, "  (%d,%d) -> %llu\n", x,y, index);
      }
    }
  ...
  writer->onEndEdgeInstances();

  writer->onEndGraphInstance();
